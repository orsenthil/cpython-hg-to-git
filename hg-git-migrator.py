...
* Install hg-git (http://hg-git.github.io/) extension for mercurial

Usage
-----

* python3 hg-repo  git@github.com:orsenthil/cpython.git

The repository (git@github.com:orsenthil/cpython2.git) should be empty.
"""
...
def print_result(cmd, result):
    ...
    print("{command} execution had an error [retcode: {retcode}].".format(command=cmd, retcode=result.returncode))
...
def get_push_revs(num_revs):
    sub_revs = []
    start_rev = REF_PUSH_LIMIT
    while num_revs > (2 * REF_PUSH_LIMIT):
        sub_revs.append(start_rev)
        start_rev += REF_PUSH_LIMIT
        num_revs -= REF_PUSH_LIMIT
    return list(reversed(sub_revs))
...
def hg_branch_to_bookmark(hg_repo_abspath):
    ...
    for branch in branches:
        ...
        execute_with_dir_context(hg_repo_abspath, "hg bookmark -r default {bookmark}".format(bookmark=bookmark))
...
def git_normalize_branches(git_repo_abspath):
    ...
    for branch in branches:
        ...
        if branch.startswith("py-"):
            ...
            execute_with_dir_context(git_repo_abspath, "git branch -m {oldname} {newname}".format(oldname=branch,
                                                                                                  newname=newname))
...
@log
def git_push_intervals(git_repo_abspath):
...
if __name__ == '__main__':
    ...
    parser.add_argument("hg_repo", help="location of the local mercurial repository.")
